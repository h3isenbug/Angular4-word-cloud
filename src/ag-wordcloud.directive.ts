...
export class AgWordCloudDirective implements OnInit {
  ...
  private buildSVG() {
    this.svg = D3.select(this.element.nativeElement)
      ...
      .attr(
        'height',
        Number(this.height) +
          Number(this.options.margin.top) +
          Number(this.options.margin.bottom)
      )
      .append('g')
      .attr(
        'transform',
  ...
  private populate() {
    ...
    const fontFace: string =
      this.options.settings.fontFace == null
        ? 'Roboto'
        : this.options.settings.fontFace;
    const fontWeight: string =
      this.options.settings.fontWeight == null
        ? 'normal'
        : this.options.settings.fontWeight;
    const spiralType: string =
      this.options.settings.spiral == null
        ? 'archimedean'
        : this.options.settings.spiral;
    d3.layout
      .cloud()
      ...
      .on('mouseover', (d, i) => {
        return this.options.labels
          ? tooltip
              ...
              .text('Size: ' + self.getWordSize(d.text))
  ...
  removeElementsByClassName(classname: string) {
    const elements = this.element.nativeElement.getElementsByClassName(
      classname
    );
